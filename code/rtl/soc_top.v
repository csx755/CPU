// =============================================================================
// soc_top �? 单周�? CPU SoC 顶层模块
// 基于 schematic (1).pdf, 接口文档�? doc/12_SoC接口文档.md
// Nexys4 A7-100T (xc7a100tcsg324-1)
// =============================================================================

module soc_top(
    input           clk,            // 100MHz 板载时钟 (pin E3)
    input           rstn,           // 复位按钮, 低有�? (pin C12)
    input  [4:0]    btn_i,          // 5 个按�?
    input  [15:0]   sw_i,           // 16 个拨码开�?
    output [7:0]    disp_an_o,      // 数码管位�?
    output [7:0]    disp_seg_o,     // 数码管段�?
    output [15:0]   led_o           // 16 �? LED
);
...
// =============================================================================
// ROM �? 指令存储�? (Distributed Memory Generator IP, 1024×32-bit)
// =============================================================================
ROM U_ROM_D (
    .a          (PC[11:2]),         // 字地�? (1024 �? = 4KB)
    .spo        (inst_in)           // 指令输出 [31:0]
);

// =============================================================================
// RAM_B �? 数据存储�? (Block Memory Generator IP, 1024×32-bit)
// =============================================================================
...
// =============================================================================
// dm_ctrl �? 数据存储器访问控制器 (手写模块, 替换�? .edf 黑盒)
// =============================================================================
...
// =============================================================================
// Multi_8CH32 �? 8 通道显示多路选择�? (老师 .edf 黑盒)
// =============================================================================
wire [31:0] Disp_num;
...
Multi_8CH32 U_Multi_8CH32 (
    ...
    .Switch         (SW_OK[7:5]),      // 参照标准答案, 不取�?
    .point_in       ({clkdiv, clkdiv}),// [已知问题] 32�?64 位宽不匹�?,复制拼接
    .LES            ({64{1'b1}}),       // �?有�?�道全亮
    .data0          (CPU2IO),          // 外设数据
    .data1          ({2'b0, PC[31:2]}),// PC 字地�?
    .data2          (inst_in),         // 当前指令
    .data3          (counter_out),     // 计数器�??
    .data4          (Addr_out),        // ALU 地址
    .data5          (Data_out),        // 写数�?
    .data6          (Cpu_data4bus),     // MIO_BUS 返回�? CPU 的数�? (参照标准答案)
    .data7          (PC),              // PC 完整�?
    .point_out      (point_out),
    .LE_out         (LE_out),
    .Disp_num       (Disp_num)
